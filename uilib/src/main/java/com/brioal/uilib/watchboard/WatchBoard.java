package com.brioal.uilib.watchboard;
...
public class WatchBoard extends View {
    ...
    private float mPadding = SizeUtil.Dp2Px(getContext(), 10); //边距
    private float mTextSize = SizeUtil.Sp2Px(getContext(), 16); //文字大小
    private int mColorLong = Color.argb(225, 0, 0, 0); //长线的颜色
    private int mColorShort = Color.argb(125, 0, 0, 0); //长线的颜色
    private int mHourPointColor = Color.BLACK; //时针的颜色
    private int mMinutePointColor = Color.BLACK; //分针的颜色
    private int mSecondPointColor = Color.RED; //秒针的颜色
    private float mHourPointWidth = SizeUtil.Dp2Px(getContext(), 5); //时针宽度
    private float mMinutePointWidth = SizeUtil.Dp2Px(getContext(), 3); //分针宽度
    private float mSecondPointWidth = SizeUtil.Dp2Px(getContext(), 2); //秒针宽度
    private int mPointRadius = (int) SizeUtil.Dp2Px(getContext(), 10); // 指针圆角
    private float mPointEndLength; //指针长度
    ...
    private void paintScale(Canvas canvas) {
        ...
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                ...
                mPaint.setColor(Color.BLACK);
                canvas.save();
                canvas.translate(0, -mRadius + SizeUtil.Dp2Px(getContext(), 5) + lineWidth - (textBound.bottom + textBound.top) / 2 + SizeUtil.Dp2Px(getContext(), 10));
                canvas.rotate(-6 * i);
                mPaint.setStyle(Paint.Style.FILL);
